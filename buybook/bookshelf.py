...
import logging
import curl_converter

logger = logging.getLogger(__name__)

class Basebookshelf(object):
    def __init__(self, session: Session, _url_auth: str, _url_bookshelf:str):
        self.wanted_books: pd.DataFrame = None
        self.url_auth = _url_auth
        self.url_bookshelf = _url_bookshelf
        self.s = session

    def authenticate(self) -> bool:
        (h, b, ct) = curl_converter.load("cURL.cmd")
        req:Request= None
...
class Readmooshare_bookself(Basebookshelf):
    def __init__(self, session):
        Basebookshelf.__init__(self, session,
                               _url_auth ="https://member.readmoo.com/login",
                               _url_bookshelf="https://share.readmoo.com/me/bookshelf/total")
